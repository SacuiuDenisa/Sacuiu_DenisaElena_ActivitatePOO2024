...
int main() {
	CofetarieOnline co1;
	cout << co1.getNume();  //apelare geter //apelare metoda //nu merge daca ar fi protected ca nu am acces si de-aia pun public la clasa mostenire
	Cofetarie c1;
	cout << c1.getNume();  //merge apelare getter

	float* vector = new float[3] {100, 200, 300};//pt ca am vectorul de salarii alocam spatiu; initializare in line
	CofetarieOnline co2("Cofetarie.ro", 4, "Cofetarie", 3, vector, 1, 2020, 10);
	CofetarieOnline co3(co2); //apelare const de copiere

	CofetarieOnline co5=co2; // const de copiere
	CofetarieOnline co4;//operator =
	co4 = co2;
	cout << co2;//apelare const de afisare
	cout << co4;

	int nrColaboratori = (int)co2;
	cout << endl << "Nr colaboratori= " << nrColaboratori << endl;
	cout << "Nr mediu de angajati pt fiecare colaborator: " << co2.getNrMediuAngajatiPerColaborator() << endl;;
