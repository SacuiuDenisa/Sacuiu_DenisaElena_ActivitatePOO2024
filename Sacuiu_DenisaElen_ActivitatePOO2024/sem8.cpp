...
private:
	Magazin magazin; //nu apelam ca e obiect 
	Cofetarie* cofetarie;//apelam
	int nrMasiniParcate;
	Masina* masiniParcate; //apelam

	//constructori getteri setter etc

public:

	Magazin getMagazin() const { //pun const ca sa nu mai modific
		return this->magazin;
	}

	//constructor fara parametrii
	ComplexComercial() {
		this->cofetarie = new cofetarie("Delicii", 3, true, 2024, 20); //daca e de tip pointer trb sa apelez eu constructorul 
		this->nrMasiniParcate = 3;
		this->masiniParcate = new Masina[this->nrMasiniParcate];
	}
	//const cu parametrii
	ComplexComercial(Magazin mag, Cofetarie* cof, int nrMaisini, Masina* masini) {
		this->magazin = mag; //cand avem operatorul = se va apela op =
		// nu merge pt ca e pointer this->cofetarie = cof;
		this->cofetarie = new Cofetarie(*cof);
		this->nrMasiniParcate = nrMasini;
		//cand avpelam cu [] e vector si cand apelam cu () apelam obiect
		this->masiniParcate = new Masina[nrMasini];
		for (int i = 0; i < nrMasini; i++) {
			this->masiniParcate[i] = masini[i]; //copiez masina cu masina din vectorulMasina* masini
			//relatie de has a ?
		}
	}
	//const de copiere 
	ComplexComercial(const ComplexComercial& cc) {
		this->magazin = cc.magazin; //cand avem operatorul = se va apela op =
		...
		for (int i = 0; i < cc.nrMasiniParcate; i++) {
			this->masiniParcate[i] = cc.masiniParcatemasini[i]; //copiez masina cu masina din vectorulMasina* masini
			//relatie de has a ?
		}
	}
	~ComplexComercial() {
...
friend ostream& operator<<(ostream& o, ComplexComercial& cc) {
	o << "Magazinul din complexul comercial" << endl;
	o << cc.magazin << endl;//se va apela operatorul << din Magazin
	o << "Cofetaria din complex este : " << endl;
	o << *cc.cofetarie << endl; //pt cae pointer trb dereferentiat ca sa nu-mi afiseze adresa 
	o << "Avem " << cc.nrMasiniParcate << "Masini parcate in complex: " << endl;
	for (int i = 0; i < cc, nrMainiParcate; i++) {
...
ComplexComercial operator--() {//forma post  fixata pt ca nu avem nicun parametru si va returna val modificata
	if (this->nrMasiniParcate > 0) {
		this->nrMasiniParcate--;
		Masina* aux = new Masina[this->nrMainiParcate]; //se apeleaza 
		for (int i = 0; i, this->nrMasiniParcate; i++) {
			aux[i] = this->masiniParcate[i]; //se apeleaza op = din clasa masina 
		}
		delete[]this->masiniParcate; //se va apela destructorul din clasa Masina de cate masini aveam inainte adica 5 
		this->masiniParcate = aux; //se va atribui op egal pt pointer 
...
ComplexComercial operator--()(int){ //pre  decrementare returnam  val nemodificat dar sa ramana modificat

	ComplexComercial cc = *this; //salvez starea initiala;
	--(*this);
	return cc;
}
...
void main() {
	...
	cout << cc2;
	cc1 = cc2--; //post  retuneaza starea initiala adica tot 5 
